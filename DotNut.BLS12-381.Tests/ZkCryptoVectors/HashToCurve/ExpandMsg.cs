...
public class ExpandMsg
{
    ...
    {
        var EXPAND_LEN = 16;
        var b1 = new byte[EXPAND_LEN];
        var b2 = new byte[EXPAND_LEN];

        ExpandMsgXmd
            .Create([.. "sig"u8, .. "nature"u8], [], EXPAND_LEN, HashAlgorithmName.SHA256)
            .ReadInto(b1);
        ExpandMsgXmd.Create("signature"u8, [], EXPAND_LEN, HashAlgorithmName.SHA256).ReadInto(b2);

        Assert.Equal(b1, b2);
    }

    // From <https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-hash-to-curve-16#appendix-K.1>
    public static TheoryData<string, int, string> Draft16TestVectors() =>
        new()
        {
            {
                "",
    ...
    [Theory]
    [MemberData(nameof(Draft16TestVectors))]
    public void expand_msg_xmd_works_for_draft16_testvectors_sha256(
        string message,
        int lenInBytes,
        string uniformBytes
    )
    {
        var dst = "QUUX-V01-CS02-with-expander-SHA256-128"u8;
        var msg = Encoding.UTF8.GetBytes(message);
        var actual = new byte[lenInBytes];
        ExpandMsgXmd.Create(msg, dst, lenInBytes, HashAlgorithmName.SHA256).ReadInto(actual);
        var expected = Convert.FromHexString(uniformBytes);
        Assert.Equal(actual, expected);
    }

    // From <https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-hash-to-curve-16#appendix-K.2>
    public static TheoryData<string, int, string> Draft16TestVectorsLongDst() =>
        new()
        {
            { "", 0x20, "e8dc0c8b686b7ef2074086fbdd2f30e3f8bfbd3bdf177f73f04b97ce618a3ed3" },
            { "abc", 0x20, "52dbf4f36cf560fca57dedec2ad924ee9c266341d8f3d6afe5171733b16bbb12" },
            {
                "abcdef0123456789",
                0x20,
    ...
    [Theory]
    [MemberData(nameof(Draft16TestVectorsLongDst))]
    public void expand_msg_xmd_works_for_draft16_testvectors_sha256_long_dst(
        string message,
        int lenInBytes,
        string uniformBytes
    )
    {
        var dst =
            "QUUX-V01-CS02-with-expander-SHA256-128-long-DST-1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111"u8;
        var msg = Encoding.UTF8.GetBytes(message);
        var actual = new byte[lenInBytes];
        ExpandMsgXmd.Create(msg, dst, lenInBytes, HashAlgorithmName.SHA256).ReadInto(actual);
        var expected = Convert.FromHexString(uniformBytes);
    ...
    public static TheoryData<string, int, string> Draft16TestVectorsSha512 =>
        new()
        {
            { "", 0x20, "6b9a7312411d92f921c6f68ca0b6380730a1a4d982c507211a90964c394179ba" },
            { "abc", 0x20, "0da749f12fbe5483eb066a5f595055679b976e93abe9be6f0f6318bce7aca8dc" },
            {
                "abcdef0123456789",
                0x20,
    ...
    [Theory]
    [MemberData(nameof(Draft16TestVectorsSha512))]
    public void expand_msg_xmd_works_for_draft16_testvectors_sha512(
        string message,
        int lenInBytes,
        string uniformBytes
    )
    {
        var dst = "QUUX-V01-CS02-with-expander-SHA512-256"u8;
        var msg = Encoding.UTF8.GetBytes(message);
        var actual = new byte[lenInBytes];
        ExpandMsgXmd.Create(msg, dst, lenInBytes, HashAlgorithmName.SHA512).ReadInto(actual);
        var expected = Convert.FromHexString(uniformBytes);
        Assert.Equal(actual, expected);
    }

    // From <https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-hash-to-curve-16#appendix-K.4>
    public static TheoryData<string, int, string> Draft16TestVectorsShake128 =>
        new()
        {
            { "", 0x20, "86518c9cd86581486e9485aa74ab35ba150d1c75c88e26b7043e44e2acd735a2" },
            { "abc", 0x20, "8696af52a4d862417c0763556073f47bc9b9ba43c99b505305cb1ec04a9ab468" },
            {
                "abcdef0123456789",
                0x20,
    ...
    [Theory]
    [MemberData(nameof(Draft16TestVectorsShake128))]
    public void expand_msg_xof_works_for_draft16_testvectors_shake128(
        string message,
        int lenInBytes,
        string uniformBytes
    )
    {
        var dst = "QUUX-V01-CS02-with-expander-SHAKE128"u8;
        var msg = Encoding.UTF8.GetBytes(message);
        var actual = new byte[lenInBytes];
        ExpandMsgXof.Create(msg, dst, lenInBytes, XofAlgorithm.Shake128).ReadInto(actual);
        var expected = Convert.FromHexString(uniformBytes);
        Assert.Equal(actual, expected);
    }

    // From <https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-hash-to-curve-16#appendix-K.5>
    public static TheoryData<string, int, string> Draft16TestVectorsShake128LongDst =>
        new()
        {
            { "", 0x20, "827c6216330a122352312bccc0c8d6e7a146c5257a776dbd9ad9d75cd880fc53" },
            { "abc", 0x20, "690c8d82c7213b4282c6cb41c00e31ea1d3e2005f93ad19bbf6da40f15790c5c" },
            {
                "abcdef0123456789",
                0x20,
    ...
    [Theory]
    [MemberData(nameof(Draft16TestVectorsShake128LongDst))]
    public void expand_msg_xof_works_for_draft16_testvectors_shake128_long_dst(
        string message,
        int lenInBytes,
        string uniformBytes
    )
    {
        var dst =
            "QUUX-V01-CS02-with-expander-SHAKE128-long-DST-111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111"u8;
        var msg = Encoding.UTF8.GetBytes(message);
        var actual = new byte[lenInBytes];
        ExpandMsgXof.Create(msg, dst, lenInBytes, XofAlgorithm.Shake128).ReadInto(actual);
        var expected = Convert.FromHexString(uniformBytes);
    ...
    public static TheoryData<string, int, string> Draft16Shake256 =>
        new()
        {
            { "", 0x20, "2ffc05c48ed32b95d72e807f6eab9f7530dd1c2f013914c8fed38c5ccc15ad76" },
            { "abc", 0x20, "b39e493867e2767216792abce1f2676c197c0692aed061560ead251821808e07" },
            {
                "abcdef0123456789",
                0x20,
    ...
    [Theory]
    [MemberData(nameof(Draft16Shake256))]
    public static void expand_msg_xof_works_for_draft16_testvectors_shake256(
        string message,
        int lenInBytes,
        string uniformBytes
    )
    {
        var dst = "QUUX-V01-CS02-with-expander-SHAKE256"u8;
        var msg = Encoding.UTF8.GetBytes(message);
        var actual = new byte[lenInBytes];
        ExpandMsgXof.Create(msg, dst, lenInBytes, XofAlgorithm.Shake256).ReadInto(actual);
        var expected = Convert.FromHexString(uniformBytes);
        Assert.Equal(actual, expected);
    }
}
