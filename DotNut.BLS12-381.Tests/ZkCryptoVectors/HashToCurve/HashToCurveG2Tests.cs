...
public class HashToCurveG2Tests
{
    ...
    [Theory]
    [MemberData(nameof(Draft16G2Nu))]
    public void encode_to_curve_works_for_draft16_testvectors_g2_sha256_nu(string msg, string expected)
    {
        var dst = "QUUX-V01-CS02-with-BLS12381G2_XMD:SHA-256_SSWU_NU_"u8;
        var g = DotNut.BLS12_381.HashToCurve.HashToCurve.EncodeToG2(Encoding.UTF8.GetBytes(msg), dst);
        Assert.True(G2Affine.TryFromUncompressed(Convert.FromHexString(expected), out var decoded));
        Assert.Equal(decoded, g);
    }

    // From <https://datatracker.ietf.org/doc/html/draft-irtf-cfrg-hash-to-curve-16#appendix-J.10.1>
    public static TheoryData<string, string> Draft16G2Ro => new()
    {
        {
            "", 
    ...
    [Theory]
    [MemberData(nameof(Draft16G2Ro))]
    public void hash_to_curve_works_for_draft16_testvectors_g2_sha256_ro(string msg, string expected)
    {
        var dst = "QUUX-V01-CS02-with-BLS12381G2_XMD:SHA-256_SSWU_RO_"u8;
        var g = DotNut.BLS12_381.HashToCurve.HashToCurve.HashToG2(Encoding.UTF8.GetBytes(msg), dst);
        Assert.True(G2Affine.TryFromUncompressed(Convert.FromHexString(expected), out var decoded));
        Assert.Equal(decoded, g);
    }
