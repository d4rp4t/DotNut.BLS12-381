...
namespace DotNut.BLS12_381.Tests.ZkCryptoVectors.VectorFiles;

public class VectorFilesTests
{
    [Fact]
    public void g1_uncompressed_valid_test_vectors()
    {
        ...
        byte[] allExpectedBytes = File.ReadAllBytes(path);

        var generatedBytesList = new List<byte>();
        var currentPoint = G1Projective.Infinity;

        ReadOnlySpan<byte> expectedBytesSpan = allExpectedBytes;

        for (int i = 0; i < 1000; i++)
        {
            var affinePoint = currentPoint.ToAffine();
            byte[] encoded = affinePoint.ToUncompressed();
            generatedBytesList.AddRange(encoded);

            var expectedChunk = expectedBytesSpan.Slice(i * encoded.Length, encoded.Length);

            Assert.True(G1Affine.TryFromUncompressed(expectedChunk, out var decoded));
            Assert.Equal(affinePoint, decoded);

            currentPoint = G1Projective.Add(currentPoint, G1Projective.Generator);
        }

        Assert.Equal(allExpectedBytes, generatedBytesList);
    }

    [Fact]
    public void g1_compressed_valid_test_vectors()
    {
        var path = Path.Combine(
            AppContext.BaseDirectory,
            "ZkCryptoVectors",
        ...
        {
            var affinePoint = currentPoint.ToAffine();
            byte[] encoded = affinePoint.ToCompressed();
            generatedBytesList.AddRange(encoded);

            var expectedChunk = expectedBytesSpan.Slice(i * encoded.Length, encoded.Length);

            Assert.True(G1Affine.TryFromCompressed(expectedChunk, out var decoded));
            Assert.Equal(affinePoint, decoded);
    ...
    [Fact]
    public void g2_uncompressed_valid_test_vectors()
    {
        var path = Path.Combine(
            AppContext.BaseDirectory,
            "ZkCryptoVectors",
        ...
        var generatedBytesList = new List<byte>();
        var currentPoint = G2Projective.Infinity;
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            Assert.True(G2Affine.TryFromUncompressed(expectedChunk, out var decoded));
            Assert.Equal(affinePoint, decoded);

            currentPoint = G2Projective.Add(currentPoint, G2Projective.Generator);
        }
    ...
    [Fact]
    public void g2_compressed_valid_test_vectors()
    {
        var path = Path.Combine(
            AppContext.BaseDirectory,
            "ZkCryptoVectors",
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            Assert.True(G2Affine.TryFromCompressed(expectedChunk, out var decoded));
            Assert.Equal(affinePoint, decoded);
    ...
    [Fact]
    public void test_pairing_result_against_relic()
    {
        var a = G1Affine.Generator;
        var b = G2Affine.Generator;

        var res = Bls12Pairing.Pair(a, b);

        var prep = G2Prepared.From(b);

        Assert.Equal(
            res,
            Bls12Pairing.MultiMillerLoop([(a, prep)]).FinalExponentiation()
        );
