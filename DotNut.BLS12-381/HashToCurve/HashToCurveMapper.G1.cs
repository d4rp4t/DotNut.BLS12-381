...
internal static partial class HashToCurveMapper
{
    ...
    internal static Fp SSWU_ELLP_A = new Fp([
    ...
    internal static Fp SSWU_ELLP_B = new Fp([
    ...
    internal static Fp SSWU_XI = new Fp([
    ...
    internal static Fp SQRT_M_XI_CUBED = new Fp([
    ...
    /// <summary>
    /// Maps a field element to a point on BLS12-381 G1 using Simplified SWU (RFC 9380 §6.6.2)
    /// followed by the 11-isogeny (RFC 9380 §E.2).
    /// Does NOT clear the cofactor; call <see cref="G1Projective.ClearCofactor"/> afterwards
    /// to obtain a point in the prime-order subgroup.
    /// </summary>
    public static G1Projective MapToCurve(this Fp u)
    {
        var pt = SswuMapG1(u);
        return IsoMapG1(pt);
    }
    ...
    /// <summary>
    /// Simplified SWU map onto the 11-isogenous curve E' (A' = <see cref="SSWU_ELLP_A"/>,
    /// B' = <see cref="SSWU_ELLP_B"/>, ξ = <see cref="SSWU_XI"/>). Returns a projective
    /// point on E', not on the BLS12-381 G1 curve.
    /// </summary>
    internal static G1Projective SswuMapG1(Fp u)
    {
        var usq    = Fp.Square(u);
        var xiUsq  = Fp.Multiply(SSWU_XI, usq);
        var xisqU4 = Fp.Square(xiUsq);
        var ndCommon = Fp.Add(xisqU4, xiUsq);    // ξ²u⁴ + ξu²
        ...
        // g(x0) as a fraction: gx0Num / gxDen  (gxDen = x_den³)
        var xDenSq = Fp.Square(xDen);
        var gxDen  = Fp.Multiply(xDenSq, xDen);
        var gx0Num = Fp.Add(
            Fp.Multiply(Fp.Add(Fp.Square(x0Num), Fp.Multiply(SSWU_ELLP_A, xDenSq)), x0Num),
            Fp.Multiply(SSWU_ELLP_B, gxDen));    // x0_num³ + A' x0_num x_den² + B' x_den³

        // sqrt candidate via u·v · (u·v³)^((p−3)/4), where u=gx0Num, v=gxDen
        var uv = Fp.Multiply(gx0Num, gxDen);
        var vsq = Fp.Square(gxDen);
        var sqrtCandidate = Fp.Multiply(uv, AdditionChains.Pm3Div4(Fp.Multiply(uv, vsq)));

        // gx0 is a square iff sqrtCandidate² · gxDen == gx0Num
        var gx0Square = Fp.CtEqual(Fp.Multiply(Fp.Square(sqrtCandidate), gxDen), gx0Num);

        var x1Num = Fp.Multiply(x0Num, xiUsq);   // x1_num = x0_num · ξu²
    ...
    /// <summary>
    /// Evaluates the degree-11 isogeny φ: E' → G1 on a projective point using Horner's
    /// rule in projective coordinates (RFC 9380 §E.2). Computes xNum, xDen, yNum, yDen
    /// simultaneously; the result is returned as a projective point with
    /// X = xNum·yDen, Y = yNum·xDen, Z = xDen·yDen.
    /// </summary>
    internal static G1Projective IsoMapG1(G1Projective u)
    {
        // z^1 .. z^15  (max polynomial degree among the four coefficient arrays is 15)
        var zPows = new Fp[15];
        zPows[0] = u.Z;
        for (var i = 1; i < zPows.Length; i++)
            zPows[i] = Fp.Multiply(zPows[i - 1], u.Z);

        // Horner's rule: mapVals[i] = sum_{j=0}^{n} c[j] · (X/Z)^j  (projective)
        var coeffs  = new Fp[][] { Iso11Xnum, Iso11Xden, Iso11Ynum, Iso11Yden };
        var mapVals = new Fp[4];
        for (var i = 0; i < 4; i++)
        {
            var c    = coeffs[i];
            var last = c.Length - 1;
            ...
            for (var j = 0; j < last; j++)
                mapVals[i] = Fp.Add(Fp.Multiply(mapVals[i], u.X), Fp.Multiply(zPows[j], c[last - 1 - j]));
        }

        // xDen has one fewer degree than xNum → multiply by an extra z
        mapVals[1] = Fp.Multiply(mapVals[1], u.Z);
        // yNum is multiplied by the affine y = Y/Z (absorbed into projective Y)
